...
type Sqlmock interface {
	...
	// метод для добавления строки в кэш
	AddRow(tableName string, rows *Rows, key interface{})

	// метод для добавления строки в кэш
	DeleteRow(tableName string, key interface{})
}
...
func (c *sqlmock) query(query string, args []namedValue) (*ExpectedQuery, error) {
	...
	matched, _ := regexp.Compile("(SELECT .* FROM) `(.*)` WHERE(.*)")
	strings := matched.FindStringSubmatch(query)
	if len(strings) > 3 {
		queries, isExist := c.table[fmt.Sprintf("%s", strings[2])]
		if isExist {
			...
			for _, v := range args {
				...
				temp, isExist := queries[fmt.Sprintf("%v", v.Value)]
				if isExist {
					sets = temp
					rows = append(rows, sets.rows[0])
				}
			}
			...
			if len(rows) > 0 {
				...
				expected.rows = &rowSets{sets: []*Rows{&sets}, ex: expected}
				return expected, nil
			}
		}
	}
...
// sql driver.Value slice or from the CSV string and
// to be used as sql driver.Rows.
func (c *sqlmock) AddRow(tableName string, rows *Rows, key interface{}) {

	_, exist := c.table[tableName]
	if !exist {
		c.table[tableName] = make(map[string]Rows)
	}

	c.table[tableName][fmt.Sprintf("%v", key)] = *rows
}

// NewRows allows Rows to be created from a
// sql driver.Value slice or from the CSV string and
// to be used as sql driver.Rows.
func (c *sqlmock) DeleteRow(tableName string, key interface{}) {
	...
	_, exist = c.table[tableName][fmt.Sprintf("%v", key)]
	if !exist {
		return
	}

	delete(c.table[tableName], fmt.Sprintf("%v", key))
}
