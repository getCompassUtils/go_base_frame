...
func New(options ...func(*sqlmock) error) (*sql.DB, Sqlmock, error) {
	...
	smock, exist := pool.conns[dsn]
	if exist {
		return smock.SqlDb, smock, nil
	}

	smock = &sqlmock{dsn: dsn, drv: pool, ordered: true, table: make(map[string]map[string]Rows)}
